<template>
  <div v-if="this.cliente">
    <h4>Cliente</h4>
    <div><label>Nome: </label> {{this.cliente.nome}}</div>
...
export default {
  ...
  methods: {
      ...
      http
      .put("/cliente/" + this.cliente.id, data) .then(response => {
...
deleteCliente() {
     http
.delete("/cliente/" + this.cliente.id) .then(response => {
        console.log(response.data);
